...
class Filesystem implements FileUrlGeneratorContract
{
    ...
    public function __construct(ApplicationContract $app,
        FilesystemFactoryContract $files
    )
    {
        $this->app = $app;
    ...
    : string
    {
        ...
        } elseif (app('files')->extension($filename) === 'gif'
            && empty($extra)
        ) {
            return $this->makeUrl($filename);
        }

        return $this->validateImageAnd($filename,
            function (string $filename) use ($extra) {
                return $this->validateProcessAnd($filename, $extra,
                    function (Image $image, array $extra = []) use ($filename) {
                        if ($extra['blur']) {
                        ...
                        return $this->putProcessFile(
                            $image,
                            $this->makeProcessFilename($filename,
                                $this->makeProcessFingerprint($extra))
                        );
                    });
            });
    }
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    private function putProcessFile(Image $image, string $filename)
    : string
    {
    ...
    protected function processSize(Image $image, array $extra)
    {
        ...
        if (($minSide === $processWidth && $processWidth)
            || ((bool) $processWidth && ! $processHeight)
        ) {
            $image->resize($processWidth, null,
                function (Constraint $constraint) {
                    $constraint->aspectRatio();
                });
        } elseif (($minSide === $processHeight && $processWidth)
            || ((bool) $processHeight && ! $processWidth)
        ) {
            $image->resize(null, $processHeight,
                function (Constraint $constraint) {
                    $constraint->aspectRatio();
                });
        }
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    private function validateProcessAnd(string $filename,
        array $extra,
        callable $call
    )
    : string
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    private function validateFingerprint(string $filename,
        callable $call,
        array $extra
    )
    : string
    {
        $processFilename = $this->makeProcessFilename($filename,
            $this->makeProcessFingerprint($extra));
        ...
        return $call(
            $this->makeImage($this->app['config']['filesystems.disks.public.root']
                .'/'.$filename),
            $extra
        );
    }
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    protected function makeImage(string $filename)
    : Image
    {
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    protected function makeProcessFingerprint(array $extra)
    : string
    {
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    protected function makeProcessFilename(string $filename,
        string $fingerprint
    )
    : string
    {
        $processPath = str_replace(sprintf('.%s',
            $ext = pathinfo($filename, PATHINFO_EXTENSION)), '/', $filename);
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    public function getSupportMimeTypes()
    : array
    {
        $mimes = [
            'image/jpeg',
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    private function validateImageAnd(string $filename, callable $call)
    : string
    {
        if (in_array($this->files->mimeType($filename),
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    protected function makeUrl(string $filename)
    : string
    {
        if ($this->disk === 'local') {
            return sprintf('%s/%s', config('cdn.generators.filesystem.public'),
                $filename);
        }
