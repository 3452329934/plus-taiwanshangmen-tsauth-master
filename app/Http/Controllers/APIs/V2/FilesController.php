...
class FilesController extends Controller
{
    ...
    public function show(Request $request, ResponseContract $response, CdnUrlManager $cdn, FileWithModel $fileWith)
    {
        ...
        $url = Cache::remember(sprintf('file_url_%d_%s', $fileWith->id, implode('_', $extra)), 50,
            function () use ($fileWith, $cdn, $extra) {
    ...
    public function store(
        StoreUploadFileRequest $request,
        ResponseContract $response,
        Carbon $dateTime,
        FileModel $fileModel,
        FileWithModel $fileWith
    ) {
        $clientHeight = $request->input('height', 0);
        $clientWidth = $request->input('width', 0);
        $fileModel = $this->validateFileInDatabase($fileModel, $file = $request->file('file'),
            function (UploadedFile $file, string $md5) use (
                $fileModel,
                $dateTime,
                $clientWidth,
                $clientHeight,
                ...
                [$width, $height] = ($imageInfo = @getimagesize($file->getRealPath())) === false ? [
                    null, null
                ] : $imageInfo;
                $path = $dateTime->format('Y/m/d/Hi');
                if (($filename = $file->store($path, config('cdn.generators.filesystem.disk'))) === false) {
                    return $response->json(['message' => trans('messages.upload_failed')], 500);
                }
                if (Str::startsWith($clientMimeType, 'image/') && $clientMimeType !== 'image/gif') {
                    ini_set('memory_limit', '-1');
                    $realPath = Storage::disk(config('cdn.generators.filesystem.disk'))->path($filename);
                    Image::make($realPath)->orientate()->save($realPath, 100);
                }
