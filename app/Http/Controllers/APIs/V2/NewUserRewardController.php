...
class NewUserRewardController extends Controller
{
    ...
    public function store(Request $request, User $target, UserProcess $processer)
    {
        ...
        if (! $amount || $amount < 0) {
            return response()->json(['amount' => trans('messages.reward_amount_err', ['gold_name' => $this->goldName])], 422);
        }

        if ($user->id == $target->id) {
            return response()->json(['message' => trans('messages.noself')], 422);
        }

        if (! $user->currency || $user->currency->sum < $amount) {
            return response()->json(['message' => trans('messages.reward_balance_err')], 403);
        }

        if (! $target->currency) {
            return response()->json(['message' => trans('messages.reward_target_err', ['gold_name' => $this->goldName])], 500);
        }
        ...
        if ($pay && $paid) {
            ...
            return response()->json(['message' => trans('messages.reward_success')], 201);
        } else {
            Cache::forget(sprintf(CacheNames::REWARD_USER_LOCK, $target->id, $user->id));
