...
class CurrentUserController extends Controller
{
    ...
    {
        $user = $request->user();
        $user->load('newWallet', 'currency', 'extra');
    ...
    public function update(Request $request, ResponseFactoryContract $response)
    {
        ...
        $rules = [
            'name' => [
                ...
                'username',
                'display_length:2,12',
                'not_in:'. (string) setting('user', 'keep-username', ''),
     ...
     * @param Request $request
     * @param ResponseFactoryContract $response
     * @param VerificationCodeModel $model
     * @return mixed
     * @throws ValidationException
     * @author Seven Du <shiweidu@outlook.com>
     */
    public function updatePhoneOrMail(Request $request, ResponseFactoryContract $response, VerificationCodeModel $model)
    {
        ...
        return $user->save()
            ? $response->make('', 204)
            : $response->json(['message' => [trans('messages.failed')]], 500);
    }
