...
class UserCertificationController extends Controller
{
    ...
    )
    {
        ...
        return $certification->getConnection()->transaction(function () use (
            ...
            return $response->json(['message' => trans('messages.audit_wait')])
                ->setStatusCode(201);
        });
    }
    ...
    )
    {
        ...
        if ($certification->status === 1) {
            return $response->json(['message' => trans('messages.audit_already')],
                422);
        }
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    protected function findNotWithFileModels(Request $request,
        FileWithModel $fileWithModel
    )
    : Collection
    {
