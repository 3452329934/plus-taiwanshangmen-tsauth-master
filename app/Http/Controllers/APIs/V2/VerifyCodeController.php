...
class VerifyCodeController extends Controller
{
    ...
    protected function sendFromRequest(Request $request)
    {
        ...
        foreach ($map as $channel => $input) {
            ...
            try {
                ...
                return response()->json(['message' => trans('messages.send_failed')], 400);
            }
            break;
        }

        return response()->json(['message' => [trans('messages.send_success')]], 202);
    }
    ...
    protected function validateSent(string $account)
    {
        ...
        if ($verify) {
            abort(403, trans('messages.verifycode_wait', ['second' => $verify->makeSurplusSecond($vaildSecond)]));
        }
    }
}
