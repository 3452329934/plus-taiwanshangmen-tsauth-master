...
class CertificationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Certification::query()
            ->when($keyword !== null, function (Builder $query) use ($keyword) {
    ...
    public function passCertification(Request $request, Certification $certification)
    {
        ...
        if ($certification->user) {
            Cache::forget(sprintf('cache_for_certification_of_%d', $certification->user_id));
                          ...
                          FileWithModel $fileWithModel)
    {
        ...
        if (! $userId) {
            return response()->json(['message' => '用户ID不存在'], Response::HTTP_UNPROCESSABLE_ENTITY);
        }

        $exists = Certification::query()->where('user_id', $userId)->exists();
        if ($exists) {
            return response()->json(['message' => '该用户已提交过认证'], Response::HTTP_UNPROCESSABLE_ENTITY);
        }
