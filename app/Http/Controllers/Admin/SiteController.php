...
class SiteController extends Controller
{
    ...
    public function updateSiteInfo(Request $request, Configuration $config, ResponseFactory $response)
    {
        ...
        $rules = [
            'name' => ['bail','nullable', 'string','display_length:0,30','not_regex:@<script\b[^>]*>[\s\S]*<\/script>@'],
            'keywords'=>['bail','nullable','string','display_length:0,30','not_regex:@<script\b[^>]*>[\s\S]*<\/script>@'],
            'description'=>['bail','nullable','string','display_length:0,200','not_regex:@<script\b[^>]*>[\s\S]*<\/script>@'],
            'icp'=>['bail','nullable','string','display_length:0,20','not_regex:@<script\b[^>]*>[\s\S]*<\/script>@'],
            'hot_tags'=>['bail','nullable','string','display_length:0,100','not_regex:@<script\b[^>]*>[\s\S]*<\/script>@'],
            'lottery_link'=>['bail','nullable','string','display_length:0,500','not_regex:@<script\b[^>]*>[\s\S]*<\/script>@']
        ];
        $messages = [
            'name.string' => '应用名称类型错误',
            'name.display_length' => '应用名称长度不合法',
            'name.not_regex' => '应用名称包含非法字符',
            'keywords.string' => '关键字类型错误',
            'keywords.display_length' => '关键字长度不合法',
            'keywords.not_regex' => '关键字包含非法字符',
            'description.string' => '描述类型错误',
            'description.display_length' => '描述长度不合法',
            'description.not_regex' => '描述包含非法字符',
            'icp.string' => 'icp类型错误',
            'icp.display_length' => 'icp长度不合法',
            'icp.not_regex' => 'icp包含非法字符',
            'hot_tags.string' => '热门搜索关键词类型错误',
            'hot_tags.display_length' => '热门搜索关键词长度不合法',
            'hot_tags.not_regex' => '热门搜索关键词包含非法字符',
            'lottery_link.string' => '中奖消息链接类型错误',
            'lottery_link.display_length' => '中奖消息链接长度不合法',
            'lottery_link.not_regex' => '中奖消息链接包含非法字符',
        ];
     ...
     * @throws \Throwable
     */
    public function updateSiteConfigure(Request $request): \Illuminate\Http\JsonResponse
    {
        setting('user')->set([
