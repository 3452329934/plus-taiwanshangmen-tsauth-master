...
class UserController extends Controller
{
    ...
    public function users(Request $request)
    {
        ...
        $showRole = (bool)$request->has('show_role');
        ...
        /*参数验证-start*/
        if($request->query('location') && !is_string($request->query('location'))){
            return response()->json([
                'errors' => ['区域参数类型错误'],
            ])->setStatusCode(403);
        }
        ...
        $rules = [
            'follow'=>['bail','numeric','in:0,1,2'],
            'role'=>['bail','numeric',Rule::in(array(0)+Role::all()->keyBy('id')->keys()->toArray()),],
            'perPage'=>['bail','numeric','display_length:0,2'],
            'phone' => ['bail','nullable','numeric','display_length:0,6'],
            'page' => ['bail','nullable','display_length:0,2'],
            'email' => ['bail','nullable','max:128'],
            'regist_start_date' => ['bail','nullable','date'],
            'regist_end_date' => ['bail','nullable','date'],
        ];
        $messages = [
            'follow.numeric'=>'请求的follow必须是整数',
            'follow.in'=>'请求的follow值不在合法范围',
            'role.numeric'=>'请求的角色必须是整数',
            'role.in'=>'请求的角色不在合法范围',
            'perPage.numeric'=>'此参数必须是数值',
            'perPage.display_length'=>'参数长度不在合法范围',
            'phone.numeric' => '请求的手机号必须是数值',
            'phone.display_length' => '请求的手机号长度不在合法范围',
            'page.display_length' => '参数长度不在合法范围',
            'email.max' => '请求的邮箱地址太长，应小于128字节',
            'regist_start_date.date' => '开始日期不合法',
            'regist_end_date.date' => '结束日期不合法',
        ];
    ...
    public function store(Request $request)
    {
        ...
        return $user->getConnection()->transaction(function () use ($request, $user) {
            $user->save();
    ...
    public function showUser(Request $request, User $user)
    {
        ...
        $showRole = (bool)$request->query('show_role');
