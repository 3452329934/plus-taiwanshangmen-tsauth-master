...
class Cash extends Process
{
    ...
    : CurrencyOrderModel {
        ...
        $amount = intval(($amount / $ratio)) * $ratio;
     ...
     * @author BS <414606094@qq.com>
     */
    public function createFailOrder(int $owner_id, int $amount)
    : CurrencyOrderModel {
        $user = $this->checkUser($owner_id);

        $title = '积分提取失败';
        $body = sprintf('提取积分失败：返还 %s %s%s', $amount,
            $this->currency_type->get('unit'), $this->currency_type->get('name'));
        ...
        // return DB::transaction(function () use ($order, $user) {
        $user->currency->increment('sum', $order->amount);
        $order->save();
