...
use Zhiyi\Plus\Requests\Feed\TopicIndex;
...
class Topic extends Controller
{
    ...
    : JsonResponse {
        ...
        $topic = $topicModel
            ->query()
            ->where('name', $topicModel->name)
            ->first();
        if ($topic && $topic->status === 'passed') {
            throw new UnprocessableEntityHttpException(sprintf('“%s”话题已存在',
                $topicModel->name));
        } else {
            if ($topic) {
                throw new UnprocessableEntityHttpException(sprintf('“%s”话题待审核中',
                    $topicModel->name));
            }
        }
        ...
        $topic = $user->getConnection()->transaction(function () use (
            ...
            $topicModel->creator_user_id = $user->id;
            $topicModel->followers_count = 1;
            $topicModel->status = setting('feed', 'topic:need-review', false)
