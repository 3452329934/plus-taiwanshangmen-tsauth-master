...
class TopicApplicationRecordController extends Controller
{
    ...
    public function accept(Request $request, TopicApplicationModel $topicApplication, Carbon $datetime, TopicModel $topicModel)
    {
        ...
        $topicApplication->user->notify(new SystemNotification(sprintf('你申请创建的专题%s已被管理员通过', $topicApplication->name), [
            'type' => 'qa:question-topic:accept',
            'topic' => $topicModel,
        ]));
    ...
    public function reject(Request $request, TopicApplicationModel $topicApplication, Carbon $datetime)
    {
        ...
        $topicApplication->user->notify(new SystemNotification(sprintf('你申请创建的专题%s已被管理员驳回', $topicApplication->name), [
            'type' => 'qa:question-topic:reject',
