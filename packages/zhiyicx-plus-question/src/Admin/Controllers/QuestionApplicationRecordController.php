...
class QuestionApplicationRecordController extends Controller
{
    ...
    public function index(
        Request $request,
        QuestionApplicationModel $questionApplicationModel
    ) {
        ...
        $query = $questionApplicationModel->when($question,
            function ($query) use ($question) { // 根据问题id查询
            ...
            ->when($endDate, function ($query) use ($endDate) { // 根据截至时间筛选
                ...
                return $query->from('questions')
                    ->whereRaw('questions.id = question_application.question_id')
        ...
        $applications = $query->offset($offset)->limit($limit)
            ->with(['question', 'user', 'examiner'])->orderBy('id', 'desc')
            ->get();
    ...
    public function accept(
        Request $request,
        QuestionApplicationModel $application,
        Carbon $datetime
    ) {
        $user = $request->user();
        ...
        $application->getConnection()->transaction(function () use ($application
            ...
            $application->user->notify(new SystemNotification(sprintf('你的问题%s已被管理员加精',
                $application->question->subject), [
                'type'        => 'qa:question-excellent:accept',
    ...
    public function reject(
        ...
        $application->getConnection()->transaction(function () use (
            $application,
            $user
        ) {
            $process = new UserProcess();
            $process->reject(0, $application->amount, $user->id,
                trans('plus-question::questions.application.退还问题申精积分'),
                trans('plus-question::questions.application.支付问题《:subject》的申精积分',
                    ['subject' => $application->question->subject]));

            $application->save();
        });

        $application->user->notify(new SystemNotification(sprintf('问题%s的加精申请已被管理员拒绝',
            $application->question->subject), [
            'type'        => 'qa:question-excellent:reject',
            'application' => $application,
        ]));
