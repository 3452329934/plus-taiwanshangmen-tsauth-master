...
class QuestionController extends Controller
{
    ...
    public function index(
        ...
        $map = [
            'all'       => function (Builder $query) {
                $query->orderBy('id', 'desc');
            },
            'new'       => function (Builder $query) {
                $query->where('answers_count', 0)
                    ->orderBy('id', 'desc');
            },
            'hot'       => function (Builder $query) use ($questionModel) {
            ...
            'reward'    => function (Builder $query) {
                $query->where('amount', '!=', 0)
                    ->orderBy('id', 'desc');
            },
            'excellent' => function (Builder $query) {
                $query->where('excellent', '!=', 0)
                    ->orderBy('id', 'desc');
            },
            'follow'    => function (Builder $query) use ($userID) {
                $query->whereExists(function (QueryBuilder $query) use
                ($userID) {
        ...
        $type = in_array($type = $request->query('type', 'new'),
            array_keys($map)) ? $type : 'new';
        ...
        call_user_func($map[$type], $query = $questionModel
            ...
            ->when($subject && ! $id, function (Builder $query) use ($subject) {
        ...
        return $response->json($questions->map(function (QuestionModel $question
            ...
            if ($question->answer) {
                /* 设置匿名用户 */
                if ($question->answer->anonymity
                    && $question->answer->user_id !== $userID
                ) {
                    $question->answer->addHidden('user');
                    $question->answer->user_id = 0;
                }
                $question->answer->liked
                    = (bool) $question->answer->liked($userID);
                $question->answer->collected
                    = (bool) $question->answer->collected($userID);
                $question->answer->rewarded
                    = (bool) $question->answer->rewarders()
                    ->where('user_id', $userID)->first();
                $question->look && $question->answer->could = true;

                if ($question->look && $question->answer->invited
                    && (! $question->answer->onlookers()
                        ->where('user_id', $userID)->first())
                    && $question->answer->user_id !== $userID
    ...
    public function show(
        ...
        $answerResolveCall = function (AnswerModel $answer) use (
            $userID,
            $question
        ) {
            ...
            $answer->onlookers_total = $answer->onlookers()->withPivot('amount')
                ->sum('amount');

            if ($question->look
                && (! in_array($userID,
                    $answer->onlookers->pluck('id')->toArray()))
            ...
            if ($userID) {
                ...
                $answer->rewarded = (bool) $answer->rewarders()
                    ->where('user_id', $userID)->first();
            }
        ...
        if ($userID) {
            $loadMap['watchers'] = function ($query) use ($userID) {
                $query->withTrashed()->where('id', $userID);
            };
        }

        $question->load($loadMap);
        $question->addHidden(['answers', 'watchers']);
        $question->watched = ! $userID
            ? false
            : (bool) $question->watchers()->newPivotStatementForId($userID)
                ->first();
        $question->my_answer = $question->answers()->with('user')
            ->where('user_id', $userID)->first();
        $question->invitation_answers
            = $question->answers->map($answerResolveCall);
    ...
    public function store(
        ...
        $usersIDs = Arr::pluck((array) $request->input('invitations', []),
            'user');

        $images = $this->findMarkdownImageNotWithModels($body ?: '');

        if ($automaticity && ! $amount) {
            return $response->json(['amount' => [trans('plus-question::questions.回答自动入账必须设置悬赏总额')]],
                422);
        } elseif ($automaticity && count($usersIDs) !== 1) {
            return $response->json(['invitations' => [trans('plus-question::questions.回答自动入账只能邀请一人')]],
                422);
        } elseif ($look && ! $automaticity) {
            return $response->json(['automaticity' => [trans('plus-question::questions.开启围观必须设置自动入账')]],
                422);
        } elseif ($look && ! $amount) {
            return $response->json(['amount' => [trans('plus-question::question.开启围观必须设置悬赏金额')]],
                422);
        }

        // Find topics.
        $topics = empty($topicsIDs) ? collect()
            : $topicModel->whereIn('id', $topicsIDs)->get();

        // Find users.
        $users = empty($usersIDs) ? collect()
            : $userModel->whereIn('id', $usersIDs)->get();
        ...
        $charge->body = trans('plus-question::questions.发布悬赏问答《%s》',
        ...
        try {
            ...
            $user->getConnection()->transaction(function () use (
                ...
                $topicModel->whereIn('id', $topicsIDs)
                    ->increment('questions_count', 1);
    ...
    public function update(
        ...
        foreach (
            array_filter($request->only(['subject', 'body', 'text_body'])) as
            $key => $value
        ) {
            $question->$key = $value;
        }
        ...
        if ($amount) {
            if ($question->user_id !== $user->id) {
                return $response->json(['message' => [trans('plus-question::questions.not-owner')]],
                    403);
            }
            if (($question->amount !== 0 && $question->invitations()->first())
                || $question->answers()->where('adoption', 1)->first()
            ) {
                return $response->json(['message' => [trans('plus-question::questions.该问题无法设置悬赏')]],
                    403);
            }
        }

        $question->getConnection()->transaction(function () use (
            $question,
            $images,
            $topicsIDs,
            $user,
            ...
            if ($topicsIDs) {
                ...
                $topicModel->whereIn('id', $question->topics->pluck('id'))
                    ->decrement('questions_count', 1);
    ...
    public function destory(
        ...
        $user->getConnection()->transaction(function () use ($user, $question) {
            // 如果有采纳或是自动入账已完成 则不退款
            if (($question->automaticity
                    && ! $question->answers()->where('invited', 1)->first())
                || ($question->amount > 0 && $question->status != 1
                    && ! $question->answers()->where('adoption', 1)->first())
            ) {
            ...
            if ($applylog = $question->applications()->where('status', '!=', 2)
                ->first()
            ) {
                ...
                $charge->subject
                    = trans('plus-question::questions.application.退还问题申精积分');
                $charge->body
                    = trans('plus-question::questions.application.退还问题《:subject》的申精积分',
                    ['subject' => $question->subject]);
                $charge->status = 1;
