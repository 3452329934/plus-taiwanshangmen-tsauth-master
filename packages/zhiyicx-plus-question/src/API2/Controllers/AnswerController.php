...
class AnswerController extends Controller
{
    ...
    public function index(
        ...
        $orderType = in_array(
            $orderType = $request->query('order_type', 'default'),
            array_keys($orderMap)) ? $orderType : 'default';

        $answers = $question->answers()
            ->whereDoesntHave('blacks', function (Builder $query) use ($userID) {
        ...
        return $response->json($answers->map(function (AnswerModel $answer) use
        (
            $userID
        ) {
            ...
            if ($userID) {
                ...
                $answer->rewarded = $answer->rewarders()
    ...
    public function show(
        ...
        if ($userID) {
            $answer->liked = (bool) $answer->likes()->where('user_id', $userID)
                ->first();
            $answer->collected = (bool) $answer->collectors()
        ...
        // 无法查看.需要围观。
        if ($answer->question && $answer->question->look
        ...
        if ($answer->question && $answer->question->anonymity
            && $answer->question->user_id !== $userID
        ) {
            $answer->question->addHidden('user');
            $answer->question->user_id = 0;
        }

        $answer->question->has_adoption = $answer->question->answers()
            ->where('adoption', 1)->first() ? true : false;
    ...
    public function store(
        ...
        $answer->invited = in_array($user->id,
                $question->invitations->pluck('id')->toArray())
            && ! $question->answers()
                ->where('invited', 1)
        ...
        $question->getConnection()->transaction(function () use (
            ...
            if ($question->automaticity && $answer->invited) {
                ...
                $charge->subject
                    = trans('plus-question::answers.charges.invited.subject');
                $charge->body
                    = trans('plus-question::answers.charges.invited.body',
    ...
    public function update(
        ...
        if ($user->id !== $answer->user_id) {
            return $response->json(['message' => [trans('plus-question::answers.not-own')]],
                403);
        } elseif ($answer->adoption) {
            return $response->json(['message' => [trans('plus-question::answers.adopted')]],
                422);
        }
        ...
        $answer->getConnection()->transaction(function () use (
            $answer,
            $images
        ) {
        ...
        return $response->json(['message' => [trans('plus-question::messages.success')]],
            201);
    }
    ...
    public function destory(
        ...
        if ($user->id !== $answer->user_id) {
            ...
            return $response->json(['message' => [trans('plus-question::answer.delete.invited')]],
                422);
        }
    ...
    public function newStore(
        ...
        $question->getConnection()->transaction(function () use (
            ...
            if ($question->automaticity && $answer->invited) {
                ...
                $order = $process->createOrder($user, $question->amount, 1,
                    trans('plus-question::answers.charges.invited.subject'),
                    trans('plus-question::answers.charges.invited.body',
                        ['body' => $question->subject]), $question->user_id);
        ...
        $question->user->notify(new System($message, [
            'user'     => $user,
