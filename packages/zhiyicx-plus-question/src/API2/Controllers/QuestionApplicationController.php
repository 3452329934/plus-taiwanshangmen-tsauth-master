...
class QuestionApplicationController extends Controller
{
    ...
    public function store(
        ...
        if ($question->applications()->where('expires_at', null)->first()) {
            return response()->json(['message' => [trans('plus-question::questions.application.already-exists')]],
                422);
        }

        if ($user->wallet->balance < $this->apply_amount) {
            return response()->json(['message' => [trans('plus-question::questions.Insufficient balance')]],
                422);
        }
        ...
        $user->getConnection()->transaction(function () use (
            ...
            $charge->subject
                = trans('plus-question::questions.application.支付问题申精积分');
            $charge->body
    ...
    public function newStore(
        ...
        $user->getConnection()->transaction(function () use (
            ...
            $process->prepayment($user->id, $this->apply_amount, 0,
                trans('plus-question::questions.application.支付问题申精积分'),
                trans('plus-question::questions.application.支付问题《:subject》的申精积分',
                    ['subject' => $question->subject]));
