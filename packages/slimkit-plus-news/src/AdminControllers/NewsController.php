...
use Zhiyi\Plus\Packages\Currency\Processes\User as Processor;
...
class NewsController extends Controller
{
    ...
    public function getNewsList(Request $request, Carbon $datetime)
    {
        ...
        $datas = News::where(function ($query) use ($key) {
            ...
            ->orderBy('id', 'desc')
            ->with([
                'user', 'tags', 'pinned' => function ($query) use ($datetime) {
    ...
    public function doSaveNews(Request $request, TagModel $tagModel)
    {
        ...
        $tags = $tagModel->whereIn('id',
            is_array($request->input('tags')) ? $request->input('tags')
                : explode(',', $request->input('tags')))->get();
        if (! $tags) {
        ...
        if ($request->news_id) {
            ...
            if ($news) {
                ...
                $news->getConnection()->transaction(function () use (
                    $news,
                    $images,
                    $tags
                ) {
    ...
    public function auditNews(Request $request, int $news_id)
    {
        ...
        if ($news) {
            ...
            switch ($request->state) {
                case 3:
                    $channel = 'news:reject';
                    $message = sprintf('您的投稿《%s》已被驳回', $news->title);
                    $news->audit_count = $news->audit_count + 1;

                    $news->getConnection()->transaction(function () use ($news
                    ) {
                        if ($news->audit_count >= 3
                            && $news->contribute_amount > 0
                        ) {  // 驳回三次退款
                            ...
                            // $charge->subject = '退还资讯投稿积分';
                            // $charge->body = sprintf('退还资讯《%s》的投稿积分',
                            //     $news->title);
                            // $charge->status = 1;
                            //
                            // $news->user->wallet->increment('balance',
                            //     $charge->amount);
                            // $charge->save();

                            $processor = new Processor();
                            $processor->reject(0, $news->contribute_amount,
                                $news->user_id,
                                '退还资讯投稿积分',
                                sprintf('退还资讯《%s》的投稿积分',
                                    $news->title)
                            );
                        }
