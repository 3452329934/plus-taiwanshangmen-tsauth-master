...
class NewsController extends Controller
{
    ...
    public function index(Request $request, News $newsModel)
    {
        ...
        $is_pinned = $request->input('is_pinned',1); //是否显示置顶数据
        ...
        $news = $newsModel
            ...
            ->when(!$is_pinned,function ($query){
                $ids = News::where('news.audit_status', 0)
                    ->join('news_pinneds', function ($join){
                        return $join->on('news_pinneds.target', '=', 'news.id')->where('channel', 'news')->where('expires_at', '>', new Carbon);
                    })->pluck('news.id');
        ...
        $data = $newsModel->getConnection()->transaction(function () use ($news, $userId) {
            return $news->each(function ($data) use ($userId) {
                ...
                $data->is_pinned = (bool) $data->pinned()->where('state', 1)->where('expires_at', '>', new Carbon)->exists();
                unset($data->pinned);
            });
        });
