    ...
    public function index(Request $request)
    {
        ...
        $user->load(['newsCollections' => function ($query) use ($after, $limit) {
            return $query->when($after, function ($query) use ($after) {
                ...
                ->select(['news.comment_count','news.audit_status', 'news.id', 'news.title', 'news.subject', 'news.created_at', 'news.updated_at', 'news.storage', 'news.cate_id', 'news.from', 'news.author', 'news.user_id', 'news.hits'])
                ->with('user')
