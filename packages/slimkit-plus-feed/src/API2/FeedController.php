...
class FeedController extends Controller
{
    ...
    public function getPinnedFeeds(
        ...
        if ($request->query('after') || $request->query('type') === 'hot'
            ...
            function () use ($feedModel, $datetime, $user) {
                return $feedModel->newQuery()->select('feeds.*')
                    ...
                    ->when($user, function (Builder $builder) use ($user) {
                        $builder->whereDoesntHave('blacks',
    ...
    public function new(
        ...
        count($updateValues) > 0 && Batch::update(new Feed(), $updateValues, 'id');
    ...
    public function store(StoreFeedPostRequest $request)
    {
        ...
        $response = $user->getConnection()->transaction(function () use (
            ...
            $fileWiths && $this->saveFeedFileWith($fileWiths, $feed);
    ...
    protected function makeFileWith(StoreFeedPostRequest $request)
    {
        ...
        return (is_countable($images) && count($images)) ? FileWithModel::query()->whereIn(
            'id',
            collect($images)->filter(function (array $item) {
    ...
    protected function makeVideoWith(StoreFeedPostRequest $request)
    {
        ...
        return isset($video['video_id']) && $video['video_id'] ? FileWithModel::query()->where(
            'id',
            $video['video_id']
        )->where('channel', null)
            ->where('raw', null)
    ...
    {
        $cover = $request->input('video');

        return isset($cover['cover_id']) && $cover['cover_id'] ? FileWithModel::query()->where(
            'id',
            $cover['cover_id']
        )->where('channel', null)
            ->where('raw', null)
