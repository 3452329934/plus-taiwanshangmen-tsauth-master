...
class CommentController extends Controller
{
    ...
    public function show(
        ...
        $commentModel = $commentModel->with(['user'])
            ...
            ->when($target_user,
                function ($query) use ($target_user) {  // 根据资源（动态）作者id查询
            ...
            ->when(($top && $top !== 'all' && ! $pinned_etime
                && ! $pinned_stime),
                function ($query) use ($pinned_type, $top, $datetime) {
                    $method = $top == 'yes' ? 'whereExists' : 'whereNotExists';

                    return $query->{$method}(function ($query) use (
                        $datetime,
                        $pinned_type
                    ) {
                        if ($pinned_type !== 'all') {
            ...
            ->when($pinned_etime, function ($query) use ($pinned_etime) {
                return $query->whereExists(function ($query) use ($pinned_etime
                    ...
                    return $query->from('feed_pinneds')
                        ->whereRaw('feed_pinneds.target = comments.id')
                        ->where('channel', 'comment')
                        ->whereDate('expires_at', '<=', $pinned_etime);
                });
            })
            ->orderBy('id', 'desc');
    ...
    public function delete(Comment $comment)
    {
        ...
        try {
            ...
            if ($pinnedComment) {
                ...
                $process->reject(0, $pinnedComment->amount,
                    $pinnedComment->user_id, '评论申请置顶退款', '退还在动态申请置顶的评论的款项');
            ...
            $comment->user->notify(new System(sprintf('你的评论《%s》已被管理员删除',
                ...
                'comment' => [
                    'id'       => $comment->id,
                    'contents' => Str::limit($comment->body, 100),
                ],
            ]));
            // 统计相关动态评论数量
            Feed::query()->where('id', $comment->commentable_id)
                ->decrement('feed_comment_count');
            // TODO 用户评论统计，积分减少
            $comment->delete();
    ...
    public function accept(
        ...
        // todo 动态评论置顶通知 @done wayne
        $pinned->user->notify(new System(sprintf('你的评论《%s》已被管理员设置为置顶',
            Str::limit($comment->body, 100)), [
        ...
        return response()->json(['message' => '操作成功', 'data' => $pinned],
            201);
    }
    ...
    public function set(
        ...
        if (! $pinnedNode) {
            ...
            $pinned->raw = $comment->commentable_id;
            ...
            $pinnedNode->user->notify(new System(sprintf('你的评论《%s》已被管理员设置为置顶',
                Str::limit($comment->body, 100)), [
                'type'    => 'pinned:feed/comment',
                'state'   => 'passed',
                // 组装动态数据
    ...
    public function reject(FeedPinned $pinned)
    {
        ...
        $feed = $pinned->commentFeed;
        $processor = new UserProcess();
        $message = sprintf('你的评论《%s》已被管理员拒绝置顶', Str::limit($comment->body, 100));
        // 处理退款
        $processor->reject(0, $pinned->amount, $pinned->user_id,
            $message, $message);
        // 发送消息
        $pinned->user->notify(new System($message, [
            'type'    => 'pinned:feed/comment',
            'state'   => 'rejected',
            // 组装动态数据
            'feed'    => [
