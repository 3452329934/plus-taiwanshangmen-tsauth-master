...
class Topic extends Controller
{
    ...
    public function adminListTopics(
    ...
    public function getReviewSwitch()
    : JsonResponse
    {
    ...
    public function reviewSwitchToggle()
    : Response
    {
        $setting = setting('feed');
        $setting->set('topic:need-review',
            ! $setting->get('topic:need-review'));

        return new Response('', 204);
    }

    public function toggleReview(Request $request, TopicModel $topic)
    : Response {
        $status = $request->input('status');
        if (! in_array($status, [
            TopicModel::REVIEW_PASSED, TopicModel::REVIEW_FAILED,
            TopicModel::REVIEW_WAITING,
        ])
        ) {
