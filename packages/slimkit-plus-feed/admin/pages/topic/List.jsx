...
class List extends React.Component {
  ...
  handleRequestTopics = (query = {}) => {
    ...
    listRequest({
      params: {
        ...this.state.cacheRequestQuery,
        limit: this.state.limit, ...query
      }
    }).
        then(({ data }) => this.setState({
          ...
          cacheRequestQuery: { ...this.state.cacheRequestQuery, ...query }
            ...
            ({ response: { data = { message: '数据加载失败，请刷新页面重拾！' } } = {} }) => this.setState(
                {
                  loading: false,
                  message: {
                    open: true,
  ...
  handleCloseMessage = () => this.setState({
    message: {
      ...this.state.message,
      open: false
    }
  ...
  handleToggleTopicReview = ({ id, status, successFn }) => {
    this.setState({ submitting: true })
    toggleReviewRequest(id, status).then(() => {
      successFn()
      this.setState({
        submitting: false,
        message: { type: 'success', open: true, text: '切换成功' },
        topics: lodash.map(this.state.topics, (topic, index) => {
          if (status === 'failed') {
            this.state.topics.splice(index, 1)
          } else if (parseInt(topic.id) === parseInt(id)) {
            return { ...topic, status }
          }
        }).filter(item => (
            !!item
