    ...
    <tr slot="table-tbody-tr" v-for="item in items">
      ...
      <td>
        <input type="text" class="form-control" :value="item.name"
               @change="handleChange(item.id, $event.target.value, 'name')">
      </td>
      <td>
        <input type="number" class="form-control" :value="item.sort_by"
               @change="handleChange(item.id, $event.target.value, 'sort_by')">
      </td>
      <td>
        <select class="form-control" :value="item.status"
                @change="handleChange(item.id, $event.target.value, 'status')">
...
<script>
  ...
  export default ({
    ...
    methods: {
      ...
      handleDelete (id) {
        if (confirm('确定要删除嘛？')) {
          admin.delete(`categories/${id}`).then(response => {
            let _this = this
      ...
      handleChange (id, val, type) {
        admin.patch(
            ...
            { validateStatus: status => status === 204 }).then(response => {
