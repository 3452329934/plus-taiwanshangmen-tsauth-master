...
class GroupMemberController
{
    ...
    public function index(Request $request, GroupModel $group)
    {
        $type = in_array($request->query('type'), [
            'all', 'founder', 'manager', 'member', 'blacklist', 'audit',
            'audit_user',
        ...
        $users = $group->members()->when($type !== 'all',
    ...
    public function removeManager(
        ...
        $member = $member->find($request->route('member'));
        $user = $request->user();
    ...
    public function audit(
        ...
        try {
            ...
            if ($group->mode == 'paid') {
                if ($status === 1) {
                    // 流水记录
                    // $charge->user_id = $group->founder->user_id;
                    // $charge->channel = 'user';
                    // $charge->action = 1;
                    // $charge->amount = $group->money;
                    // $charge->subject = '用户加圈，审核通过';
                    // $charge->body = sprintf('用户%s申请加入《%s》圈子审核通过,圈主收益',
                    //     $member->user->name, $group->name);
                    // $charge->state = 1;
                    // $charge->account = $member->user_id;
                    // $charge->save();

                    /// 保存圈内收入流水
                    $income = new GroupIncomeModel();
                    $income->subject = sprintf('"%s "加入圈子',
                        $member->user->name);
                    $income->type = 1;
                    ...
                    // 用户加钱
                    $group->founder->user->newWallet()
                        ->increment('balance', $group->money);
                    // 发送通知
                    ...
                    // 流水
                    // $charge->user_id = $member->user_id;
                    // $charge->channel = 'user';
                    // $charge->action = 0;
                    // $charge->amount = $group->money;
                    // $charge->subject = '用户加圈，审核拒绝';
                    // $charge->body = sprintf('用户%s申请加入《%s》圈子审核拒绝,用户退款',
                    //     $member->user->name, $group->name);
                    // $charge->state = 1;
                    // $charge->account = $user->id;
                    // $charge->save();
                    // 用户退款
                    $member->user->newWallet()
                        ->increment('balance', $group->money);

                    // 发送通知
                    $member->user->notify(new SystemNotification('管理员已拒绝你的加入圈子申请',
                        [
                            'group' => [
                                'id'   => $group->id,
                                'name' => $group->name,
            ...
            // 保存成员审核记录
            if ($log = $member->logs()->where('status', 0)
                ->where('auditer', null)->first()
            ) {
                $log->status = $status;
                $log->auditer = $user->id;
    ...
    public function transferOwner(
        ...
        $target = $group->members()->where('user_id', $target_user_id)
            ->where('audit', 1)->where('disabled', 0)->first();
        ...
        $target_user->notify(new SystemNotification(sprintf('%s将圈子「%s」转入给你',
        ...
        try {
            if ($group->mode == 'paid') {
                ...
                if ($status === 1) {
                    ...
                    $process->receivables($group->founder->user_id,
                        $group->money, $member->user_id, '用户加圈，审核通过',
                        sprintf('用户%s申请加入《%s》圈子审核通过,圈主收益', $memberUser->name,
                            $group->name));
                    ...
                    $process = new UserProcess();
                    $process->reject($group->founder->user_id, $group->money,
                        $member->user_id, '用户加圈，审核拒绝',
                        sprintf('用户%s申请加入《%s》圈子审核拒绝,用户退款', $memberUser->name,
                            $group->name));
                }
            }
            ...
            if ($status === 1) {
                ...
                $member->save();
                $group->increment('users_count');
                $member->user->notify(new SystemNotification(sprintf('你已成功加入「%s」圈子',
                    $group->name), [
                    'group' => [
                        'id'   => $group->id,
                        'name' => $group->name,
     ...
     * @return mixed
     */
    public function roleCount(GroupModel $group)
    {
        $query = $group->members->where('audit', 1)->where('disabled', 0);

        return response()->json([
            'member_count' => $query->where('role', 'member')->count(),
            'admin_count'  => $query->whereIn('role',
                ['administrator', 'founder'])->count(),
        ], 200);
    }
}
