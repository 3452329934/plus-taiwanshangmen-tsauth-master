...
class PostCommentController
{
    ...
    public function store(StoreCommentRequest $request, GroupPostModel $post, CommentModel $commentModel)
    {
        ...
        if ($group->model != 'public' && !$member) {
            return response()->json(['message' => '你非该圈子成员请先加入圈子，当然你也可能没有该圈评论权限！'], 403);
        }
        ...
        $post->getConnection()->transaction(function () use ($post, $commentModel, $user) {
        ...
        $post->user->notify(new CommentNotification($commentModel, $user));

        $reply = UserModel::withTrashed()->find($reply);
        if ($reply) {
            $reply->notify(new CommentNotification($commentModel, $user));
        }
