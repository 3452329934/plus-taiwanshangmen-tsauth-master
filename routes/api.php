...
use Zhiyi\Plus\API2\Controllers\Comment;
use Zhiyi\Plus\API2\Controllers\QA\Answer;
use Zhiyi\Plus\API2\Controllers\Feed\Topic;
use Zhiyi\Plus\API2\Controllers\Group\Post;
use Zhiyi\Plus\API2\Controllers\Group\Group;
use Zhiyi\Plus\Http\Controllers\APIs\V2 as API2;
use Zhiyi\Plus\API2\Controllers\User\Message\At;
use Zhiyi\Plus\API2\Controllers\Feed\TopicFollow;
use Zhiyi\Plus\API2\Controllers\Feed\TopicReport;
use Zhiyi\Plus\Http\Controllers\DevelopController;
use Zhiyi\Plus\API2\Controllers\NewsPostController;
use Zhiyi\Plus\API2\Controllers\QAAnswerController;
use Zhiyi\Plus\API2\Controllers\QAQuestionController;
use Zhiyi\Plus\API2\Controllers\UserCountsController;
use Zhiyi\Plus\API2\Controllers\Feed\TopicParticipant;
use Zhiyi\Plus\API2\Controllers\NotificationController;
use Zhiyi\Plus\API2\Controllers\User\AbilityController;
use Illuminate\Contracts\Routing\Registrar as RouteContract;

Route::any('/develop', DevelopController::class.'@index');
...
Route::group(['prefix' => 'v2'], function (RouteContract $api) {
    ...
    */
    /* 读取协议 */
    $api->get('/site-agreement', API2\SystemController::class.'@getAgreement');
    /* 预览 */
    $api->get('/site-agreement-view', API2\SystemController::class.'@siteAgreement');
    $api->post('/pingpp/webhooks',
        API2\PingPlusPlusChargeWebHooks::class.'@webhook');

    $api->post('/plus-pay/webhooks',
        API2\NewWalletRechargeController::class.'@webhook');

    $api->post('/currency/webhooks',
        API2\CurrencyRechargeController::class.'@webhook');
    ...
    // 积分充值验证
    $api->post('/alipayCurrency/notify',
        API2\CurrencyPayController::class.'@alipayNotify');
    $api->post('/wechatCurrency/notify',
        API2\CurrencyPayController::class.'@wechatNotify');
    /*
    | 应用启动配置.
    */
    ...
    // Get Advertising.
    $api->get('/advertisingspace/{space}/advertising',
        API2\AdvertisingController::class.'@advertising');
    $api->get('/advertisingspace/advertising',
        API2\AdvertisingController::class.'@batch');
    ...
    */
    ...
    $api->group(['prefix' => 'verifycodes'], function (RouteContract $api) {
        ...
        */
        ...
        $api->post('/register',
            API2\VerifyCodeController::class.'@storeByRegister');
    ...
    */
    ...
    tap($api->get('/files/{fileWith}', API2\FilesController::class.'@show'),
        function ($route) {
    ...
    */
    $api->group(['prefix' => 'user'], function (RouteContract $api) {
        ...
        $api->post('/find-by-phone',
            API2\FindUserController::class.'@findByPhone');
        ...
        $api->get('/find-by-tags',
            API2\FindUserController::class.'@findByTags');
    });
    ...
    $api->group(['prefix' => 'users'], function (RouteContract $api) {
        ...
        */
        ...
        $api->get('/{user}/followers',
            API2\UserFollowController::class.'@followers');

        // 获取用户关注的用户
        $api->get('/{user}/followings',
            API2\UserFollowController::class.'@followings');
    ...
    $api->put('/user/retrieve-password',
        API2\ResetPasswordController::class.'@retrieve');
    ...
    */
    ...
    $api->group(['middleware' => 'auth:api'], function (RouteContract $api) {
        /* 保存协议 */
        $api->patch('/site-agreement', API2\SystemController::class.'@saveAgreement');
        ...
        */
        ...
        $api->group(['prefix' => 'user'], function (RouteContract $api) {
            ...
            */
            ...
            $api->put('/',
                API2\CurrentUserController::class.'@updatePhoneOrMail');

            // 查看用户未读消息统计
            // $api->get('/unread-count',
            //     API2\UserUnreadCountController::class.'@index');
            ...
            */
            ...
            // User certification.
            $api->group(['prefix' => 'certification'],
                function (RouteContract $api) {

                    // Send certification.
                    $api->post('/',
                        API2\UserCertificationController::class.'@store');

                    // Update certification.
                    $api->patch('/',
                        API2\UserCertificationController::class.'@update');

                    // Get user certification.
                    $api->get('/',
                        API2\UserCertificationController::class.'@show');
                });

            // send a feedback.
            $api->post('/feedback',
                API2\SystemController::class.'@createFeedback');

            // get a list of system conversation.
            $api->get('/conversations',
                API2\SystemController::class.'@getConversations');

            /*
            | 用户关注
             */

            $api->group(['prefix' => 'followings'],
                function (RouteContract $api) {

                    // 我关注的人列表
                    $api->get('/',
                        API2\CurrentUserController::class.'@followings');

                    // 关注一个用户
                    $api->put('/{target}', API2\CurrentUserController::class
                        .'@attachFollowingUser');

                    // 取消关注一个用户
                    $api->delete('/{target}', API2\CurrentUserController::class
                        .'@detachFollowingUser');
                });

            $api->group(['prefix' => 'followers'],
                function (RouteContract $api) {

                    // 获取关注我的用户
                    $api->get('/',
                        API2\CurrentUserController::class.'@followers');
                });

            // 获取相互关注的用户
            $api->get('/follow-mutual',
                API2\CurrentUserController::class.'@followMutual');

            // Reset password.
            $api->put('/password',
                API2\ResetPasswordController::class.'@reset');
            ...
            $api->group(['prefix' => 'tags'], function (RouteContract $api) {
                ...
                $api->delete('/{tag}',
                    API2\TagUserController::class.'@destroy');
            });

            // 打赏用户
            tap($api->post('/{target}/rewards',
                API2\UserRewardController::class.'@store'), function ($route) {
                $route->setAction(array_merge($route->getAction(), [
                    'middleware' => [
                        'cors-should',
                        'bindings',
            ...
            // 新版打赏用户
            tap($api->post('/{target}/new-rewards',
                API2\NewUserRewardController::class.'@store'),
                function ($route) {
                    $route->setAction(array_merge($route->getAction(), [
                        'middleware' => [
                            'cors-should',
            ...
            $api->post('/black/{targetUser}',
                API2\UserBlacklistController::class.'@black');
            $api->delete('/black/{targetUser}',
                API2\UserBlacklistController::class.'@unBlack');
            $api->get('/blacks',
                API2\UserBlacklistController::class.'@blackList');
        });
        ...
        */
        ...
        $api->group(['prefix' => 'wallet'], function (RouteContract $api) {
            ...
            tap($api->post('/cashes',
                API2\WalletCashController::class.'@store'), function ($route) {
                $route->setAction(array_merge($route->getAction(), [
                    'middleware' => [
                        'cors-should',
                        'bindings',
            ...
            $api->post('/recharge',
                API2\WalletRechargeController::class.'@store');
            ...
            $api->get('/charges/{charge}',
                API2\WalletChargeController::class.'@show');
        });

        // 新版支付
        $api->group(['prefix' => 'walletRecharge'],
            function (RouteContract $api) {
                // 申请凭据入口
                $api->post('/orders', API2\PayController::class.'@entry');

                // 手动检测支付宝订单的支付状态
                $api->post('/checkOrders',
                    API2\PayController::class.'@checkAlipayOrder');

                // 手动检测微信订单的支付状态
                $api->post('/checkWechatOrders',
                    API2\PayController::class.'@checkWechatOrder');
            });

        $api->group(['prefix' => 'currencyRecharge'],
            function (RouteContract $api) {
                $api->post('/orders',
                    API2\CurrencyPayController::class.'@entry');
                $api->post('/checkOrders',
                    API2\CurrencyPayController::class.'@checkAlipayOrder');
            });
        ...
        $api->group(['prefix' => 'plus-pay'], function (RouteContract $api) {
            ...
            // 发起提现申请
            tap($api->post('/cashes',
                API2\NewWalletCashController::class.'@store'),
                function ($route) {
                    $route->setAction(array_merge($route->getAction(), [
                        'middleware' => [
                            'cors-should',
            ...
            // 发起充值
            $api->post('/recharge',
                API2\NewWalletRechargeController::class.'@store');

            // 钱包订单列表
            $api->get('/orders',
                API2\NewWalletRechargeController::class.'@list');

            // 取回凭据
            $api->get('/orders/{order}',
                API2\NewWalletRechargeController::class.'@retrieve');

            // 转账
            tap($api->post('/transfer',
                API2\TransferController::class.'@transfer'), function ($route) {
                $route->setAction(array_merge($route->getAction(), [
                    'middleware' => [
                        'cors-should',
                        'bindings',
            ...
            // 转换积分
            tap($api->post('/transform',
                API2\NewWalletRechargeController::class.'@transform'),
                function ($route) {
                    $route->setAction(array_merge($route->getAction(), [
                        'middleware' => [
                            'cors-should',
        ...
        $api->get('/files/uploaded/{hash}',
            API2\FilesController::class.'@uploaded');
        ...
        $api->group(['prefix' => 'report'], function (RouteContract $api) {
            ...
            $api->post('/comments/{comment}',
                API2\ReportController::class.'@comment');
        });
        ...
        $api->group(['prefix' => 'easemob'], function (RouteContract $api) {
            ...
            // 注册环信用户(单个)
            $api->post('register/{user_id}',
                EaseMobIm\EaseMobController::class.'@createUser')
                ->where(['user_id' => '[0-9]+']);

            //批量注册环信用户
            $api->post('/register',
                EaseMobIm\EaseMobController::class.'@createUsers');

            // 为未注册环信用户注册环信（兼容老用户）
            $api->post('/register-old-users',
                EaseMobIm\EaseMobController::class.'@registerOldUsers');

            // 重置用户环信密码
            $api->put('/password',
                EaseMobIm\EaseMobController::class.'@resetPassword');

            // 获取环信用户密码
            $api->get('/password',
                EaseMobIm\EaseMobController::class.'@getPassword');
            ...
            $api->get('/groups',
                EaseMobIm\GroupController::class.'@newGetGroup');

            // 获取群头像
            $api->get('/group/face',
                EaseMobIm\GroupController::class.'@getGroupFace');

            // 添加群成员
            $api->post('/group/member',
                EaseMobIm\GroupController::class.'@addGroupMembers');

            // 移除群成员
            $api->delete('/group/member',
                EaseMobIm\GroupController::class.'@removeGroupMembers');

            // 获取聊天记录Test
            $api->get('/group/message',
                EaseMobIm\EaseMobController::class.'@getMessage');
        });
        ...
        $api->group(['prefix' => 'currency'], function (RouteContract $api) {
            // 积分流水
            $api->get('/orders',
                API2\CurrencyRechargeController::class.'@index');

            // 发起充值
            $api->post('/recharge',
                API2\CurrencyRechargeController::class.'@store');

            // 取回凭据
            $api->get('/orders/{order}',
                API2\CurrencyRechargeController::class.'@retrieve');

            // 发起提现
            $api->post('/cash', API2\CurrencyCashController::class.'@store');

            // 通过积分购买付费节点
            $api->post('/purchases/{node}',
                API2\PurchaseController::class.'@payByCurrency');

            // 调用IAP发起充值
            $api->post('/recharge/apple-iap',
                API2\CurrencyApplePayController::class.'@store');

            // IAP支付完成后的验证
            $api->post('/orders/{order}/apple-iap/verify',
                API2\CurrencyApplePayController::class.'@retrieve');

            // IAP商品列表
            $api->get('/apple-iap/products',
                API2\CurrencyApplePayController::class.'@productList');
     ...
     * 获取用户未读数信息
     */
    $api->get('/user/counts', UserCountsController::class.'@count');

    /*
     * 重置未读信息
     */
    $api->patch('/user/counts', UserCountsController::class.'@reset');
    ...
    $api->group(['prefix' => 'feed'], function (RouteContract $api) {
        ...
        $api->group(['prefix' => 'topics'], function (RouteContract $api) {
             ...
             */
            $api->patch('{topic}', Topic::class.'@update');
             ...
             * @Response::header('Status', 200, 'OK')
             */
            $api->get('{topic}', Topic::class.'@show');
             ...
             */
            $api->get('{topic}/feeds',
                Zhiyi\Plus\API2\Controllers\Feed\TopicFeed::class);
             ...
             * </pre>')
             */
            $api->get('{topic}/participants', TopicParticipant::class.'@index');
        });
    });
     ...
     * @Response::header('Status', 204, 'No Content')
     */
    $api->put('user/feed-topics/{topicID}', TopicFollow::class.'@follow');
     ...
     * @Response::header('Status', 204, 'No Content')
     */
    $api->delete('user/feed-topics/{topicID}', TopicFollow::class.'@unfollow');
     ...
     * @Response::header('Status', 204, 'No Content')
     */
    $api->put('user/report-feed-topics/{topic}', TopicReport::class);
     ...
     * </pre>')
     */
    $api->get('user/message/atme', At::class);
     ...
     * </pre>')
     */
    $api->get('comments', Comment::class.'@index');
     ...
     * ')
     */
    $api->get('group/simple-posts', Post::class.'@simpleList');
     ...
     * @Response:header('Status', 204, 'No Content')
     */
    $api->put('group/posts/{post}/toggle-excellent',
        Post::class.'@toggleExcellent');

    /*
     * Get group preview posts
     * @GET /api/v2/group/groups/:groupId/preview-posts
     */
    $api->get('group/groups/{group}/preview-posts',
        Post::class.'@previewPosts');
     ...
     * @Response::header('Status', 204, 'No Content')
     */
    $api->put('group/groups/{group}/bind-im-group',
        Group::class.'@bindImGroupId');
     ...
     * ')
     */
    $api->get('qa/reposted-answers', Answer::class.'@repostedAnswerList');

    // List all authed user abilities
    $api->get('user/abilities', AbilityController::class);

    // User Notifications
    $api->get('user/notifications', NotificationController::class.'@index');
    $api->patch('user/notifications', NotificationController::class.'@update');
    $api->get('user/notification-statistics',
        NotificationController::class.'@statistics');
    $api->patch('user/clear-follow-notification',
        NotificationController::class.'@clearFollowNotifications');

    // News Posts
    $api->delete('news/posts/{post}', NewsPostController::class.'@destroy');

    // Q&A Answers
    $api->delete('qa/answers/{answer}', QAAnswerController::class.'@destroy');

    // Q&A questions
    $api->delete('qa/questions/{question}',
        QAQuestionController::class.'@destroy');
});
